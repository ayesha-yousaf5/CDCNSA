...
Model binaries are intentionally excluded from Git. `download_models.py` downloads
the combined-model and Brinjal/Cucumber Google Drive archives during deployment,
then verifies that every required checkpoint is present and every protected hash
matches.
